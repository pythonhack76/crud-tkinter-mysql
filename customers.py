...
root = Tk()
root.title("Registro degli utenti")
...
def Database():
    global conn, cursor
    conn = pymysql.connect(host = 'localhost', user = 'root', password = 'root', db = 'dorico')
    cursor = conn.cursor()
 
def Create():
    if  FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or ADDRESS.get() == "" or USERNAME.get() == "" or PASSWORD.get() == "":
        txt_result.config(text="Si prega di compilare tutti i campi richiesti!", fg="red")
    else:
        Database()
...
def Read():
    ...
    cursor.close()
    conn.close()
    txt_result.config(text="Lettura del database con successo!", fg="black")
...
def Update():
    ...
    if GENDER.get() == "":
        ...
        tree.delete(*tree.get_children())
        cursor.execute("UPDATE `member` SET `firstname` = %s, `lastname` = %s, `gender` = %s,  `address` = %s,  `username` = %s, `password` = %s WHERE `id_member` = %s",
        ...
        txt_result.config(text="Registrazione aggiornata con successo!", fg="black")
...
def Delete():
    if not tree.selection():
       txt_result.config(text="Please select an item first", fg="red")
        ...
        result = tkMessageBox.askquestion('Cadastro Usuários', 'Sei sicuro di voler eliminare il record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
            contents =(tree.item(curItem))
            selecteditem = contents['values']
            tree.delete(curItem)
            Database()
            cursor.execute("DELETE FROM `member` WHERE `id_member` = %d" % selecteditem[0])
            conn.commit()
            cursor.close()
            conn.close()
            txt_result.config(text="Record eliminato con successo!", fg="black")
 
 
def Exit():
    result = tkMessageBox.askquestion('Registro Utenti', 'Sei sicuro di voler uscire?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
Male = Radiobutton(RadioGroup, text="Maschio", variable=GENDER, value="Male", font=('arial', 16)).pack(side=LEFT)
Female = Radiobutton(RadioGroup, text="Femmnina", variable=GENDER, value="Female", font=('arial', 16)).pack(side=LEFT)
 
#==================================LABEL WIDGET=======================================
txt_title = Label(Top, width=900, font=('arial', 24), text = "Registro Utenti")
txt_title.pack()
txt_firstname = Label(Forms, text="Nome:", font=('arial', 16), bd=15)
txt_firstname.grid(row=0, sticky="e")
txt_lastname = Label(Forms, text="Cognome:", font=('arial', 16), bd=15)
txt_lastname.grid(row=1, sticky="e")
txt_gender = Label(Forms, text="Sesso:", font=('arial', 16), bd=15)
txt_gender.grid(row=2, sticky="e")
txt_address = Label(Forms, text="Indirizzo:", font=('arial', 16), bd=15)
txt_address.grid(row=3, sticky="e")
txt_username = Label(Forms, text="Utente:", font=('arial', 16), bd=15)
...
tree.heading('Lastname', text="Cognome", anchor=W)
tree.heading('Gender', text="Sesso", anchor=W)
tree.heading('Address', text="Indirizzo", anchor=W)
tree.heading('Username', text="Utente", anchor=W)
